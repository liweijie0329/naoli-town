...
const DRAWING_AI_RUBRICS = {
  ...
  clock: [
    ...
    { key: "hands", label: "指针", detail: "两根指针大致表示 11 点 10 分即可；允许手绘角度小偏差，但需能看出分针指向 2 附近、时针在 11 附近且时针较短。" }
  ]
};
...
const tasks = [
  ...
  },
  {
    ...
    scoring: "按 MoCA 标准并考虑手绘误差：轮廓 1 分，圆、椭圆或近似圆均可；数字 1 分，1-12 基本写全、可辨认且总体顺时针即可；指针 1 分，两根指针大致表示 11 点 10 分且时针较短。"
  },
  {
    id: "naming",
    domain: "命名",
...
const rubricGroups = [
  {
    ...
    items: [
      ...
      { title: "画钟表", prompt: "请您在此处画一个钟表，填上所有的数字并指示出 11 点 10 分。", scoring: "轮廓 1 分：圆、椭圆或近似圆均可，允许轻微缺陷。数字 1 分：1-12 基本写全、可辨认、总体顺时针分布即可，允许歪斜、大小不一、间距不均。指针 1 分：必须有两个指针并大致表示 11 点 10 分，时针短于分针。", image: true }
    ]
  },
  {
    title: "命名与记忆",
...
function drawingStandardText(taskId) {
  ...
  if (taskId === "clock") return "圆/椭圆/近似圆表盘、1-12 基本写全且顺时针、两指针大致表示 11 点 10 分";
  return "";
}
...
function drawingAiRubric(task) {
  ...
  return {
    ...
    maxScore: task.maxScore,
    instruction: "请只根据用户画布图片评分，按照 MoCA 中文量表分项给出 scoreSuggestion；评分时要考虑老年人手绘误差，不要因为线条抖动、轻微歪斜、椭圆形表盘、数字大小不一或间距不均而扣分。",
    outputContract: {
    ...
    scoreRules: task.drawingKind === "cube"
      ...
      : [
        ...
        "指针：有两根指针且大致表示 11 点 10 分就给 1 分；允许角度小偏差，但应能看出分针指向 2 附近、时针在 11 附近且时针较短。",
